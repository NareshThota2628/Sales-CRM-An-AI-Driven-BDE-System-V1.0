...
const WeekView: React.FC<WeekViewProps> = ({ meetings, currentDate, now, onEventClick, onNewEvent, onEventDrop }) => {
    ...
    const handleGridClick = (e: React.MouseEvent<HTMLDivElement>, dayIndex: number) => {
        ...
        const newEventDate = new Date(days[dayIndex]);
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="grid grid-cols-[auto,1fr,1fr,1fr,1fr,1fr,1fr,1fr] sticky top-0 bg-white z-10 border-b border-slate-200">
                <div className="w-16"></div>
                {days.map(day => (
                    <div key={day.toString()} className="text-center py-2 border-l border-slate-200">
                        <p className="text-xs text-slate-500">{day.toLocaleDateString('default', { weekday: 'short' })}</p>
                        <p className="text-lg font-semibold text-slate-800">{day.getDate()}</p>
                    </div>
                ))}
            </div>
            <div className="flex-1 overflow-y-auto relative grid grid-cols-[auto,1fr,1fr,1fr,1fr,1fr,1fr,1fr]">
                <div className="w-16">
                    {hours.map(hour => (
                ...
                {days.map((day, dayIndex) => (
                    ...
                    >
                        ...
                        {meetings.filter(m => new Date(m.start).toDateString() === day.toDateString()).map(meeting => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <p className={`font-bold text-xs truncate ${status === 'ongoing' ? 'pl-2' : ''}`}>{meeting.title}</p>
