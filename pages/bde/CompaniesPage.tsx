...
const CompanyCard: React.FC<{ company: Company }> = ({ company }) => {
    ...
    return (
        ...
        >
            ...
            <div className="mt-3 text-xs bg-slate-50 p-2 rounded-md">
                <span className="font-semibold text-slate-500">Next Action: </span>
                <span className="text-slate-700">{company.nextAction || 'None Scheduled'}</span>
            </div>
        </div>
    );
};
...
const CompanyRow: React.FC<{ company: Company }> = ({ company }) => {
    ...
    return (
        <tr onClick={() => navigate(`/bde/companies/${company.id}`)} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors cursor-pointer">
            ...
            <td className="px-4 py-3 text-slate-600">{company.nextAction || 'None'}</td>
...
const CompaniesPage: React.FC = () => {
    ...
    useEffect(() => {
        const loadData = () => {
            ...
            if (userRole === 'bde') {
                ...
                setCompaniesData(allCompanies.filter(c => c.owner.id === 'usr-1'));
            } else {
    ...
    const kpiValues = useMemo(() => {
        ...
        const myCompanyIds = new Set(companiesData.map(c => c.id));
        return { totalArr, avgHealth, atRiskCount, growthOpportunities: aiInsightsData.filter(i => i.type === 'growth' && myCompanyIds.has(i.companyId)).length };
    }, [companiesData]);
        ...
        <div className="space-y-8">
            <header className="flex flex-wrap justify-between items-center gap-4 animate-fade-in">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Manage and grow your portfolio of converted accounts.</p>
                </div>
                <Button onClick={() => navigate('/bde/leads/new')} leftIcon={<PlusIcon className="w-4 h-4" />}>Add New Lead</Button>
            </header>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in" style={{ animationDelay: '100ms' }}>
             ...
             <section className="animate-fade-in" style={{ animationDelay: '200ms' }}>
                ...
                <div className="flex gap-4 overflow-x-auto pb-4">
                    {aiInsightsData.filter(i => filteredCompanies.some(c => c.id === i.companyId)).map(insight => <AiInsightCard key={insight.id} insight={insight} />)}
                </div>
            </section>
